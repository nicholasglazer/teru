...
const tools_list_body: []const u8 =
    ...
    \\{"name":"teruwm_screenshot","description":"Capture the full compositor output as PNG: terminal panes, external client windows (xdg + xwayland, popups included), and bars. Native composite — no grim needed.","inputSchema":{"type":"object","properties":{"path":{"type":"string","description":"Output path (default: /tmp/teruwm-screenshot.png)"}},"required":[]}},
    ...
    \\{"name":"teruwm_test_scroll","description":"TEST ONLY: set a pane's scrollback position in PIXELS, bypassing the wlr_pointer_axis handler (which MCP cannot synthesize). Drives sub-pixel smooth scroll (scroll_offset + scroll_pixel) deterministically; pair with teruwm_screenshot_pane to assert the 1px-per-unit render shift. Returns the resulting scroll_offset/scroll_pixel/cell_h/max_offset.","inputSchema":{"type":"object","properties":{"name":{"type":"string","description":"Pane name (e.g. term-0-1)"},"node_id":{"type":"integer"},"pixel_delta":{"type":"integer","description":"pixels to scroll; positive scrolls up into history"},"absolute":{"type":"boolean","description":"if true, treat pixel_delta as the target offset from the live bottom (resets to 0 first)"}},"required":["pixel_delta"]}},
...
const tool_table = std.StaticStringMap(F.Thunk).initComptime(.{
    ...
    .{ "teruwm_test_scroll", WmMcpTools.thunkTestScroll },
